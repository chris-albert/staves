...
export function DeviceSelect({ devices, value, onChange, kind = 'input' }: DeviceSelectProps) {
  ...
  // Position the portal menu relative to the button, flipping upward if near bottom.
  // Uses a two-pass approach: render offscreen first, measure, then position.
  useEffect(() => {
    if (!open || !buttonRef.current || !menuRef.current) return;
    const btnRect = buttonRef.current.getBoundingClientRect();
    const menuRect = menuRef.current.getBoundingClientRect();
    const spaceBelow = window.innerHeight - btnRect.bottom;
    if (spaceBelow < menuRect.height + 4) {
      setMenuPos({ top: btnRect.top - menuRect.height - 4, left: btnRect.left });
    } else {
      setMenuPos({ top: btnRect.bottom + 4, left: btnRect.left });
    }
  }, [open]);
  ...
  return (
    <>
      <button
        ...
        title={selected?.label ?? (kind === 'input' ? 'System Default Input' : 'System Default Output')}
      >
        {kind === 'input' ? (
          ...
          <svg width="8" height="8" viewBox="0 0 10 10" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
            <path d="M1 3v4l2.5 2h1V1h-1L1 3Z" />
            <path d="M6.5 3.5c.5.5.8 1 .8 1.5s-.3 1-.8 1.5" />
          </svg>
        )}
